...
class Persetujuan_prinsip extends CI_Model
{
	...
	public function __construct()
	{
		...
		log_message('debug', "Persetujuan_prinsip_model Class Initialized");
	}
	...
	public function form( $data_id = '' )
	{
		...
		return array(
			...
			array(
				...
				'label'	=> 'Nama lengkap',
				'type'	=> 'text',
				'std'	=> ($data_id != '' ? $query->{$this->slug.'_pemohon_nama'} : ''),
				'validation'=> 'required' ),
			array(
				'name'	=> $this->slug.'_pemohon_jabatan',
				'label'	=> 'Jabatan',
				'type'	=> 'text',
				'std'	=> ($data_id != '' ? $query->{$this->slug.'_pemohon_jabatan'} : ''),
				'validation'=> 'required' ),
			array(
				'name'	=> $this->slug.'_pemohon_usaha',
				'label'	=> 'Perusahaan',
				'type'	=> 'text',
				'std'	=> ($data_id != '' ? $query->{$this->slug.'_pemohon_usaha'} : ''),
				'validation'=> 'required' ),
			array(
				'name'	=> $this->slug.'_pemohon_alamat',
				'label'	=> 'Alamat',
				'type'	=> 'textarea',
				'std'	=> ($data_id != '' ? $query->{$this->slug.'_pemohon_alamat'} : ''),
				'validation'=> 'required' ),
			array(
			...
			array(
				...
				'type'	=> 'text',
				'std'	=> ($data_id != '' ? $query->{$this->slug.'_lokasi_tujuan'} : ''),
				'validation'=> 'required' ),
			array(
				'name'	=> $this->slug.'_lokasi_alamat',
				'label'	=> 'Alamat Lokasi',
				'type'	=> 'textarea',
				'std'	=> ($data_id != '' ? $query->{$this->slug.'_lokasi_alamat'} : ''),
				'validation'=> 'required' ),
			array(
				'name'	=> $this->slug.'_lokasi_nama',
				'label'	=> 'Luas Area (M<sup>2</sup>)',
				'type'	=> 'number',
				'std'	=> ($data_id != '' ? $query->{$this->slug.'_lokasi_nama'} : ''),
				'validation'=> 'required' ),
			array(
				'name'	=> $this->slug.'_lokasi_area_hijau',
				'label'	=> 'Area terbuka hijau',
				'type'	=> 'text',
				'std'	=> ($data_id != '' ? $query->{$this->slug.'_lokasi_area_hijau'} : ''),
				'validation'=> 'required' ),
			);
	}
